...
class UserController extends Controller
{
    ...
    public function update(UpdateUserRequest $request, User $user)
    {
        ...
        if ($request->hasFile('avatar')) {
            $validatedData['avatar']
                = $this->uploadAvatarAndGetPath($request);
        }
    ...
    public function destroy(User $user)
    {
        if ($user->invoices()->exists()) {
            return redirect()->back()->with('error',
                __('messages.User cannot be deleted because it has associated invoices.'));
        }
    ...
    private function uploadAvatarAndGetPath(Request $request): string|null
    {
        ...
        if ($request->hasFile('avatar')) {
            ...
            $folder
                        = date('Y-m-d'); // Get the current date as the folder name
    ...
    public function search(Request $request): JsonResponse
    {
        ...
        if ($request->has('search') && $request->search != '') {
            $users = $users->where('full_name', 'like',
                '%'.$request->search.'%');
        }
